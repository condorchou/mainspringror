  ...
  def destroy
    ...
    begin
      ...
      respond_to do |format|
        ...
        format.xml  { head :ok }
        format.json { render :json => {:deleted => true}, :callback => params[:callback], :status => :ok}
      end
    rescue => e
       respond_to do |format|
        format.json { render :json => {:errors => e.to_s}, :callback => params[:callback], :status => :unprocessable_entity}
      end
